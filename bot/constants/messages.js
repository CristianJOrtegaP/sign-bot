...
const REFRIGERADOR = {
    ...
    REQUEST_SAP_BODY: 'Ingresa el *Número SAP* del refrigerador.\n\n' +
        'Está en la etiqueta del equipo.\n\n' +
        'También puedes enviar *foto del código de barras*.',

    CONFIRM_TITLE: '📋 Refrigerador Encontrado',

    CONFIRMED: '✅ *Refrigerador confirmado*\n\n' +
...
const VEHICULO = {
    ...
    VEHICULO_REGISTERED: '✅ Vehículo registrado.\n\n' +
        'Describe el problema:\n' +
        '_Ejemplo: No enciende, hace ruido, frenos_',

    VEHICULO_REGISTERED_TITLE: '✅ Registrado',

    VEHICULO_REGISTERED_BODY: 'Describe el problema:\n' +
        '_Ejemplo: No enciende, hace ruido, frenos_',
     ...
     */
    reporteCreado: (numeroTicket, codigoSAPVehiculo, numeroEmpleado, descripcion, ubicacion = null, tiempoLlegada = null) => {
        let msg = '✅ *Reporte creado*\n\n' +
            `*Ticket:* ${numeroTicket}\n` +
    ...
    REQUEST_UBICACION: '📍 *¿Dónde te encuentras?*\n\n' +
        'Envía tu *ubicación actual* usando WhatsApp.\n\n' +
        '_Toca + o 📎 → Ubicación → Enviar ubicación actual_',

    REQUEST_UBICACION_BODY: 'Envía tu *ubicación actual* usando WhatsApp.\n\n' +
        '_Toca + o 📎 → Ubicación → Enviar ubicación actual_',
    ...
    UBICACION_INVALIDA: '❌ No pude obtener tu ubicación.\n\n' +
...
const DETECCION = {
    ...
    confirmRefrigerador: (text) =>
        `Mencionaste: "${text}"\n\n` +
        '¿Es un refrigerador?',

    CONFIRM_VEHICULO_TITLE: '¿Es un Vehículo?',
    confirmVehiculo: (text) =>
        `Mencionaste: "${text}"\n\n` +
        '¿Es un vehículo?',
    ...
    datosExtraidosRefrigerador: (equipo, problema, incluirSaludo = false) => {
        const greeting = incluirSaludo ? `${GENERAL.GREETING  }\n\n` : '';
        return `${greeting}*Refrigerador encontrado:*\n\n` +
            `*SAP:* ${equipo.CodigoSAP}\n` +
            `*Modelo:* ${equipo.Modelo}\n` +
            `*Cliente:* ${equipo.NombreCliente}\n` +
            `*Problema:* ${problema}\n\n` +
    ...
    datosExtraidosVehiculoCompleto: (numeroEmpleado, codigoSap, problema, incluirSaludo = false) => {
        const greeting = incluirSaludo ? `${GENERAL.GREETING  }\n\n` : '';
        return `${greeting}*Datos detectados:*\n\n` +
            `*Vehículo:* ${codigoSap}\n` +
            `*Empleado:* ${numeroEmpleado}\n` +
            `*Problema:* ${problema}\n\n` +
            'Creando reporte...';
...
const VALIDACION = {
    ...
    NO_ENTIENDO: 'No entendí tu mensaje.\n\n' +
        'Puedo ayudarte con:\n' +
        '• ❄️ Refrigeradores\n' +
        '• 🚗 Vehículos',
...
const CONSULTA = {
    ...
    SIN_TICKETS: '📋 No encontré reportes registrados con tu número.\n\n' +
        '¿Quieres crear un nuevo reporte?',

    TICKET_NO_AUTORIZADO: '❌ Este ticket no está asociado a tu número.\n\n' +
    ...
    detalleTicket: (ticket) => {
        ...
        if (ticket.TipoReporte === 'REFRIGERADOR') {
            if (ticket.CodigoSAP) msg += `*SAP:* ${ticket.CodigoSAP}\n`;
            if (ticket.Modelo) msg += `*Modelo:* ${ticket.Modelo}\n`;
            if (ticket.NombreCliente) msg += `*Cliente:* ${ticket.NombreCliente}\n`;
        } else if (ticket.TipoReporte === 'VEHICULO') {
            if (ticket.CodigoSAPVehiculo) msg += `*SAP Vehículo:* ${ticket.CodigoSAPVehiculo}\n`;
            if (ticket.NumeroEmpleado) msg += `*Empleado:* ${ticket.NumeroEmpleado}\n`;
        }
...
const ENCUESTA = {
    ...
    // Confirmaciones
    RESPUESTA_REGISTRADA: (numero, total) =>
        `✅ Respuesta registrada (${numero}/${total})`,
