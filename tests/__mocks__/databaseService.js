...
equipos.set('7654321', { ...sampleEquipo, EquipoId: 2, CodigoSAP: '7654321', NombreCliente: 'OXXO Centro' });
...
const mockDatabaseService = {
    ...
    getEquipoById: jest.fn().mockImplementation(async (equipoId) => {
        for (const equipo of equipos.values()) {
            if (equipo.EquipoId === equipoId) {return equipo;}
        }
        return null;
    }),
    ...
    createReporte: jest.fn().mockImplementation(async (equipoId, clienteId, telefono, descripcion, imagenUrl) => {
    ...
    createReporteVehiculo: jest.fn().mockImplementation(async (codigoSAP, numeroEmpleado, telefono, descripcion, imagenUrl) => {
        const ticket = `TKT${++ticketCounter}`;
        const reporte = {
            ReporteId: ++reporteIdCounter,
            NumeroTicket: ticket,
    ...
    updateSession: jest.fn().mockImplementation(async (telefono, estado, datosTemp, equipoIdTemp, _origenAccion, _descripcion, _reporteId) => {
    ...
    saveMessage: jest.fn().mockImplementation(async (telefono, tipo, contenido, tipoContenido, intencionDetectada, confianzaIA) => {
        messages.push({
            telefono,
            tipo,
            contenido,
