...
describe('EncuestaFlow', () => {
  ...
  describe('iniciarEncuesta', () => {
    test('debe iniciar encuesta exitosamente', async () => {
      ...
      const result = await encuestaFlow.iniciarEncuesta(testPhone, reporte, 1, tipoEncuesta, []);
  ...
  describe('handleComentario', () => {
    ...
    test('debe manejar encuestaId faltante', async () => {
      ...
      expect(whatsapp.sendText).toHaveBeenCalledWith(testPhone, expect.stringContaining('error'));
    });
  });
