...
function validateEnvVars() {
    ...
    if (process.env.NODE_ENV !== 'production') {
        ...
        if (missingOptional.length > 0) {
            console.warn('[CONFIG] WARN: Variables de entorno opcionales no definidas:', missingOptional.join(', '));
        }
    }
}
...
if (session.warningMinutes >= session.timeoutMinutes) {
    console.warn(`[CONFIG] WARN: SESSION_WARNING_MINUTES (${session.warningMinutes}) debe ser menor que SESSION_TIMEOUT_MINUTES (${session.timeoutMinutes}). Ajustando a ${session.timeoutMinutes - 5} minutos.`);
...
const audio = {
    // Habilitar transcripción de audio (requiere Azure OpenAI con gpt-4o-mini-audio)
    enabled: process.env.AUDIO_TRANSCRIPTION_ENABLED === 'true',

    // Nombre del deployment de audio en Azure OpenAI
    // Usa gpt-4o-mini-audio-preview para mejor cuota y precisión que Whisper
    audioDeployment: process.env.AZURE_AUDIO_DEPLOYMENT || 'gpt-4o-mini-audio',
