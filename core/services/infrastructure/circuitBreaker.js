...
class CircuitBreaker {
  ...
  recordSuccess() {
    ...
    if (this.state === STATES.HALF_OPEN) {
      ...
      if (this.successes >= this.config.successThreshold) {
        logger.info(
          `[CircuitBreaker:${this.name}] HALF_OPEN -> CLOSED (${this.successes} successes)`
        );
  ...
  recordFailure(error) {
    ...
    if (this.state === STATES.HALF_OPEN) {
      ...
      this.failures++;
      logger.debug(
        `[CircuitBreaker:${this.name}] Failure in CLOSED (${this.failures}/${this.config.failureThreshold})`,
        {
          error: error?.message,
        }
      );
