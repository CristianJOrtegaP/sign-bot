...
const {
  sanitizeContext,
  sanitizeForLogging: _sanitizeForLogging,
...
const logger = {
   ...
   * SEGURIDAD: Sanitiza el contexto para prevenir exposición de datos sensibles
   */
  _format(level, message, context = {}) {
    ...
    const enrichedContext = correlationId ? { correlationId, ...context } : context;

    // SEGURIDAD: Sanitizar contexto antes de loguear (solo en producción para no afectar debugging)
    const isProduction = process.env.NODE_ENV === 'production';
    const safeContext = isProduction ? sanitizeContext(enrichedContext) : enrichedContext;
...
function handleError(error, operation, context = {}) {
  // FASE 10/10: Enviar excepción a Application Insights
  try {
  ...
  if (config.database.reconnectErrorCodes.includes(error.code)) {
    logger.warn(`Error de conexión en ${operation}, reintentable`, {
      code: error.code,
      ...context,
    });
    return {
