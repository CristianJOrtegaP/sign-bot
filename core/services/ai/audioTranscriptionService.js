...
const TRANSCRIPTION_CONFIG = {
    ...
    // Formatos soportados
    supportedFormats: ['audio/ogg', 'audio/mpeg', 'audio/mp3', 'audio/mp4', 'audio/wav', 'audio/webm', 'audio/x-m4a', 'audio/opus']
};
...
function isEnabled() {
    ...
    // Usa el endpoint principal de Azure OpenAI (GPT-4o-mini-audio está en el mismo recurso)
...
function normalizeAudio(samples) {
    ...
    if (maxAbs < 0.01 || maxAbs > 0.9) return samples;
...
async function transcribeAudio(audioBuffer, options = {}) {
    ...
    try {
        ...
        const isSupported = TRANSCRIPTION_CONFIG.supportedFormats.some(f =>
            normalizedMime.includes(f.split('/')[1]) || normalizedMime.includes('ogg') || normalizedMime.includes('opus')
        );
        ...
        // Construir URL del endpoint de Azure OpenAI Chat Completions
        const audioConfig = config.audio || {};
        const endpoint = config.ai.azureOpenAI.endpoint.replace(/\/$/, '');
        const apiKey = config.ai.azureOpenAI.apiKey;
        const deploymentName = audioConfig.audioDeployment || 'gpt-4o-mini-audio';
        const apiVersion = '2025-01-01-preview';

        // Chat Completions endpoint para audio
        ...
        const audioBase64 = audioToSend.toString('base64');

        // Construir el request para Chat Completions con audio
        const requestBody = {
            messages: [
                {
                    role: 'system',
                    content: `Eres un asistente de transcripción. Tu única tarea es transcribir el audio proporcionado al español.
                    - Transcribe EXACTAMENTE lo que se dice en el audio
                    - No agregues comentarios, explicaciones ni formato adicional
                    - Si el audio está vacío o no se entiende, responde: "[Audio no claro]"
            ...
            max_tokens: 4096,
            temperature: 0  // Temperatura 0 para transcripción más precisa
        };

        logger.debug('Enviando audio a GPT-4o-mini-audio', {
        ...
        const transcribedText = response.data.choices?.[0]?.message?.content?.trim() || '';
