 ...
 * Valores optimizados para Azure Functions con carga moderada (100 reportes/día)
 */
const POOL_CONFIG = {
    min: 2,                    // Conexiones mínimas en pool (mantiene warm)
    max: 10,                   // Conexiones máximas (suficiente para 100 req/día)
    idleTimeoutMillis: 30000,  // 30s - cerrar conexiones inactivas
    acquireTimeoutMillis: 15000 // 15s - timeout para obtener conexión del pool
};
...
function isTransientError(error) {
    ...
    if (error.number && typeof error.number === 'number' && config.database.transientErrorNumbers.includes(error.number)) {
        return true;
    }
    ...
    if (error.name === 'RequestError' && (error.code === 'ETIMEOUT' || error.message?.includes('Timeout'))) {
        return true;
    }
