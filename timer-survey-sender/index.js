...
module.exports = async function (context, myTimer) {
    ...
    try {
        ...
        const reportesPendientes = await EncuestaRepository.getReportesPendientesEncuesta(minutosEspera);
        ...
        context.log(`Procesando ${reportesAProcesar.length} encuestas (max: ${MAX_ENCUESTAS_POR_EJECUCION})`);

        // Procesar cada reporte
        for (const reporte of reportesAProcesar) {
            try {
                context.log(`Enviando encuesta para ticket: ${reporte.NumeroTicket} -> ${reporte.TelefonoReportante}`);
                ...
                if (enviado) {
                    ...
                    context.log(`   Encuesta ${encuestaId} enviada: ${reporte.NumeroTicket} (tipo: ${tipoEncuesta?.Codigo || 'default'})`);
                } else {
                    stats.errores++;
